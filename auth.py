...
def load_users():
    return read_json("usuarios.json", [{"nomina":"admin","nombre":"Administrador","permiso":"Administrador","password":"admin123","activo":True}])

def save_users(users):
    write_json("usuarios.json", users)

def load_project():
    return read_json("proyecto.json", {"tiendas_proyecto":["Arco Norte","Ecatepec","Miravalle","Puebla Sur","Vallejo"],"pestanas":["Dashboard","Por Día","Reporte Semanal","Reporte Mensual","Conversión","Recuperación Económica","Productividad","Recorridos","Rankings","Macro","Diagnóstico","Configuración","Usuarios"]})

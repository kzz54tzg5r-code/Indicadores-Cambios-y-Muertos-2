...
def init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS usuarios_acceso (
...
def get_usuarios_acceso():
    init_db()
    conn = sqlite3.connect(DB_PATH)
    try:
        df = pd.read_sql_query("SELECT nomina, nombre, permiso, activo, actualizado FROM usuarios_acceso ORDER BY nombre", conn)
    except Exception:
        df = pd.DataFrame(columns=["nomina", "nombre", "permiso", "activo", "actualizado"])
    conn.close()
    return df


def guardar_usuario_acceso(nomina, nombre, permiso, password, activo=1):
    init_db()
    nomina = str(nomina).strip()
    nombre = str(nombre).strip()
    permiso = str(permiso).strip()
    password = str(password).strip()
    if not nomina or not nombre or not permiso or not password:
        raise ValueError("Completa nómina, nombre, permiso y contraseña.")
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute(
        """INSERT OR REPLACE INTO usuarios_acceso(nomina,nombre,permiso,password,activo,actualizado)
           VALUES (?,?,?,?,?,?)""",
        (nomina, nombre, permiso, password, int(activo), datetime.now().isoformat())
    )
    conn.commit()
    conn.close()


def autenticar_usuario(nomina, password, permiso_solicitado):
    init_db()
    nomina = str(nomina or "").strip()
    password = str(password or "").strip()
    permiso_solicitado = str(permiso_solicitado or "Consulta").strip()
    if not nomina or not password:
        return None
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT nomina,nombre,permiso,password,activo FROM usuarios_acceso WHERE nomina=?", (nomina,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return None
    nom, nombre, permiso, pwd, activo = row
    if int(activo or 0) != 1 or str(pwd) != password:
        return None
    jerarquia = {"Consulta": 1, "Gerente": 2, "Administrador": 3}
    if jerarquia.get(str(permiso), 0) < jerarquia.get(permiso_solicitado, 1):
        return None
    return {"nomina": nom, "nombre": nombre, "permiso": permiso}


def eliminar_usuario_acceso(nomina):
    init_db()
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("DELETE FROM usuarios_acceso WHERE nomina=?", (str(nomina).strip(),))
    conn.commit()
    conn.close()
...
def render_orion_table(df, group_day=False, max_rows=None):
    ...
    # Herramienta administrativa para ordenar / mover filas antes de mostrar la tabla.
    try:
        admin_activo = bool(globals().get("is_admin", False))
    except Exception:
        admin_activo = False

    if admin_activo and len(d) > 1:
        try:
            key_base = "tbl_" + hashlib.md5(("|".join(map(str, d.columns)) + str(len(d))).encode("utf-8")).hexdigest()[:10]
            with st.expander("🔧 Administrador: ordenar / mover filas de esta tabla", expanded=False):
                modo_orden = st.radio(
                    "Tipo de orden",
                    ["Como viene", "Ordenar por columna", "Mover fila manualmente"],
                    horizontal=True,
                    key=f"{key_base}_modo"
                )
                if modo_orden == "Ordenar por columna":
                    c1_ord, c2_ord = st.columns([2, 1])
                    with c1_ord:
                        col_ord = st.selectbox("Columna para ordenar", list(d.columns), key=f"{key_base}_col")
                    with c2_ord:
                        asc_ord = st.toggle("Ascendente", value=True, key=f"{key_base}_asc")
                    d = d.sort_values(col_ord, ascending=asc_ord, kind="stable").reset_index(drop=True)
                elif modo_orden == "Mover fila manualmente":
                    order_key = f"{key_base}_order"
                    if order_key not in st.session_state or len(st.session_state[order_key]) != len(d):
                        st.session_state[order_key] = list(range(len(d)))
                    d = d.iloc[st.session_state[order_key]].reset_index(drop=True)
                    opciones = [f"{i+1}. " + " | ".join([str(d.iloc[i][c]) for c in list(d.columns)[:2]]) for i in range(len(d))]
                    fila = st.selectbox("Fila", list(range(len(d))), format_func=lambda i: opciones[i], key=f"{key_base}_fila")
                    b1, b2, b3 = st.columns(3)
                    if b1.button("⬆️ Subir", key=f"{key_base}_up") and fila > 0:
                        orden = st.session_state[order_key]
                        orden[fila-1], orden[fila] = orden[fila], orden[fila-1]
                        st.session_state[order_key] = orden
                        st.rerun()
                    if b2.button("⬇️ Bajar", key=f"{key_base}_down") and fila < len(d)-1:
                        orden = st.session_state[order_key]
                        orden[fila+1], orden[fila] = orden[fila], orden[fila+1]
                        st.session_state[order_key] = orden
                        st.rerun()
                    if b3.button("Restablecer", key=f"{key_base}_reset"):
                        st.session_state[order_key] = list(range(len(d)))
                        st.rerun()
        except Exception as _ord_err:
            st.caption(f"No fue posible activar el orden manual: {_ord_err}")
...
with st.sidebar:
    ...
    current_user = {"nomina": "", "nombre": "Consulta", "permiso": "Consulta"}

    if rol == "Consulta":
        st.caption("Modo consulta: visualización general.")
        nomina_login = st.text_input("Nómina", key="login_nomina_consulta")
        clave_login = st.text_input("Contraseña", type="password", key="login_pass_consulta")
        usuario_db = autenticar_usuario(nomina_login, clave_login, "Consulta") if (nomina_login or clave_login) else None
        if usuario_db:
            current_user = usuario_db
            st.success(f"Usuario activo: {usuario_db['nombre']}")
        elif nomina_login or clave_login:
            st.warning("Usuario o contraseña incorrectos.")
        st.info("Permisos: solo consulta.")

    elif rol == "Gerente":
        nomina_login = st.text_input("Nómina", key="login_nomina_gerente")
        clave_login = st.text_input("Contraseña", type="password", key="login_pass_gerente")
        usuario_db = autenticar_usuario(nomina_login, clave_login, "Gerente")
        clave_gerente_master = clave_login == st.secrets.get("GERENTE_PASSWORD", "orion_gerente")
        if usuario_db or clave_gerente_master:
            is_manager = True
            can_edit_names = True
            can_view_diagnostics = True
            current_user = usuario_db or {"nomina":"GERENTE", "nombre":"Gerente", "permiso":"Gerente"}
            st.success(f"Gerente activo: {current_user['nombre']}")
        elif nomina_login or clave_login:
            st.warning("Clave de gerente incorrecta.")

    elif rol == "Administrador":
        nomina_login = st.text_input("Nómina", key="login_nomina_admin")
        clave = st.text_input("Clave administrador", type="password")
        usuario_db = autenticar_usuario(nomina_login, clave, "Administrador")
        clave_admin_master = clave == st.secrets.get("ADMIN_PASSWORD", "orion_admin")
        if usuario_db or clave_admin_master:
            is_admin = True
            can_upload = True
            can_config = True
            can_edit_names = True
            can_view_diagnostics = True
            current_user = usuario_db or {"nomina":"ADMIN", "nombre":"Administrador", "permiso":"Administrador"}
            st.success(f"Administrador activo: {current_user['nombre']}")
        elif clave:
            st.warning("Clave incorrecta.")

    st.caption(f"Rol activo: {rol}")
    if is_admin:
        st.success("Permisos: carga, metas, usuarios, nombres y diagnóstico.")
    elif is_manager:
...
def filtrar_conversion_por_periodo(conv_det_all, key_prefix):
    ...
    if semana_unica_cero:
        st.warning("No se detectó fecha comercial válida para filtrar por calendario. Se muestra el acumulado comercial disponible como Semana 0, sin aplicar filtro de fecha.")
        return d
...
if "18. Configuración de Metas" in tab:
    with tab["18. Configuración de Metas"]:
        ...
        st.caption("Como administrador puedes crear usuarios con nómina, nombre, tipo de permiso y contraseña.")
        with st.form("form_crear_usuario_acceso"):
            cu1, cu2, cu3, cu4 = st.columns([1, 2, 1.4, 1.4])
            with cu1:
                nueva_nomina = st.text_input("Nómina")
            with cu2:
                nuevo_nombre = st.text_input("Nombre")
            with cu3:
                nuevo_permiso = st.selectbox("Tipo de permiso", ["Consulta", "Gerente", "Administrador"])
            with cu4:
                nueva_pass = st.text_input("Contraseña", type="password")
            activo_usuario = st.checkbox("Usuario activo", value=True)
            guardar_usuario = st.form_submit_button("Guardar usuario")
            if guardar_usuario:
                try:
                    guardar_usuario_acceso(nueva_nomina, nuevo_nombre, nuevo_permiso, nueva_pass, 1 if activo_usuario else 0)
                    st.success("Usuario guardado correctamente.")
                    st.rerun()
                except Exception as exc:
                    st.error(str(exc))

        usuarios_df = get_usuarios_acceso()
        if not usuarios_df.empty:
            render_orion_table(usuarios_df)
            usuario_eliminar = st.selectbox(
                "Eliminar usuario",
                [""] + usuarios_df["nomina"].astype(str).tolist(),
                format_func=lambda x: "Selecciona usuario" if x == "" else x,
                key="select_eliminar_usuario_acceso"
            )
            if usuario_eliminar and st.button("Eliminar usuario seleccionado", key="btn_eliminar_usuario_acceso"):
                eliminar_usuario_acceso(usuario_eliminar)
                st.success("Usuario eliminado.")
                st.rerun()
        else:
            st.info("Aún no hay usuarios creados. Puedes seguir entrando con la clave maestra de administrador.")

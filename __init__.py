...
def nav(items):
    ...
    c0,c1,c2 = st.columns([.8,7.8,.8])
    with c0:
        if st.button("◀", key="nav_prev", use_container_width=True):
            i=items.index(st.session_state.page); st.session_state.page = items[(i-1)%len(items)]; st.rerun()
    with c1:
...
def panel(title, df=None, height=360, editable=False):
    ...
    if df is not None:
        if df.empty: st.info("Sin información.")
        elif editable: st.data_editor(format_table(df), hide_index=True, width="stretch", height=height, num_rows="dynamic")
        else: st.dataframe(format_table(df), hide_index=True, width="stretch", height=height)
    st.markdown("</div>", unsafe_allow_html=True)

def kpis(res):
    c=st.columns(5)
    vals=[("Ingresos",res.get("Ingresos",0)),("Habilitado",res.get("Acondicionado",0)),("Ubicado",res.get("Ubicado",0)),("Pendiente",res.get("Pendiente",0)),("% Ubicado",f'{res.get("% Ubicado",0):.1f}%')]
    for col,(l,v) in zip(c, vals):
        col.metric(l, f"{v:,.0f}" if isinstance(v,(int,float)) else v)

def combined_chart(df, title):
    if df is None or df.empty: return st.info("Sin información para graficar.")
    fig=go.Figure()
    ...
    fig.update_layout(title=title, barmode="group", height=430, dragmode=False, plot_bgcolor="white", paper_bgcolor="white", margin=dict(l=10,r=10,t=45,b=95), legend=dict(orientation="h", y=1.12))
...
def excel_download(df, name):
    ...
    st.download_button("Descargar Excel", b.getvalue(), name, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
